...
void __mctd_render_td_portal_headless(mc_node *node)
{
  ...
  for (int a = 0; a < node->children->count; ++a) {
    ...
    if (child->layout && child->layout->visible && child->layout->render_present) {
      // TODO fptr casting
      void (*render_node_present)(image_render_queue *, mc_node *) =
...
void __mctd_render_td_portal_present(image_render_queue *render_queue, mc_node *node)
{
  mctd_portal *td_portal = (mctd_portal *)node->data;
...
void mctd_render_cube_present(image_render_queue *render_queue, mc_node *node)
{
  ...
  glm_rotate((vec4 *)&model, 0, {0.f, 1.f, 0.f});
  {
