...
void function_editor_handle_keyboard_input_v1(frame_time *elapsed, mc_node_v1 *fedit, mc_input_event_v1 *event)
{
  ...
  switch (event->detail.keyboard.key) {
    ...
    //         char *combined = (char *)malloc(sizeof(char) * (previous_line_len + strlen(state->text.lines[state->cursorLine]) +
    //         1)); strcpy(combined, state->text.lines[state->cursorLine - 1]); strcat(combined,
