...
void MidgeApp::processStatement(MethodCall *methodCall, int nextStatementIndex)
{
  ...
  case 'b':
  {
    ...
    while (argsRemain)
    {
      ...
      string parameterString = statement.substr(ix, iy - ix);
      switch (parameterString[0])
      {
      case '"':
      {
    ...
    // Invoke
    // TODO return values
    void *retValue = method(&args[0], parameters.size());
  }
  break;
  case 'p':
  {
