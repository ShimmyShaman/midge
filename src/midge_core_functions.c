...
int instantiate_function_v1(int argc, void **argv)
{
  ...
  char decl_buf[1024];
  sprintf(decl_buf, "cm_%s = &%s;", func_info->name, func_identity_buf);
  ...
  // Code Block
  {
    ...
    for (; loop; ++i) {
      ...
      if (copy_line) {
        ...
        // 1]); printf("strlen:%zu\n", strlen(feState->text.lines[feState->text.lines_count - 1])); if (feState->text.lines_count
        // > 1)
...
int read_and_declare_function_from_editor(function_edit_info *state, function_info **defined_function_info)
{
  ...
  if (func_info) {
    ...
    // Declare with cling
    char buf[1024];
    sprintf(buf, "int (*cm_%s)(int, void **);", func_info->name);
    clint_process(buf);
  }
