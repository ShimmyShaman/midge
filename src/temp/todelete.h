...
int attach_function_info_to_owner(function_info *func_info) {
  ...
  register_midge_stack_function_entry("attach_function_info_to_owner", __FILE__, __LINE__, &midge_error_stack_index);

  mc_global_data *data;

  obtain_midge_global_root(&data);

  append_to_collection((void ***)&data->functions.items, &data->functions.alloc, &data->functions.count, (void *)func_info);
...
int attach_struct_info_to_owner(struct_info *structure_info) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("attach_struct_info_to_owner", __FILE__, __LINE__, &midge_error_stack_index);

  mc_global_data *data;

  obtain_midge_global_root(&data);

  append_to_collection((void ***)&data->structs.items, &data->structs.alloc, &data->structs.count, (void *)structure_info);
...
int attach_enumeration_info_to_owner(enumeration_info *enum_info) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("attach_enumeration_info_to_owner", __FILE__, __LINE__, &midge_error_stack_index);

  mc_global_data *data;

  obtain_midge_global_root(&data);

  append_to_collection((void ***)&data->enumerations.items, &data->enumerations.alloc, &data->enumerations.count, (void *)enum_info);
...
int initialize_parameter_info_from_syntax_node(mc_syntax_node *parameter_syntax_node, parameter_info **initialized_parameter) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("initialize_parameter_info_from_syntax_node", __FILE__, __LINE__, &midge_error_stack_index);
    ...
    case PARAMETER_KIND_STANDARD:
{
    ...
    break;    case PARAMETER_KIND_FUNCTION_POINTER:
{
      ...
      // Name
      mcs_copy_syntax_node_to_text(parameter_syntax_node->parameter.function_pointer->fptr_declarator.name, &parameter->name);
    ...
    break;    case PARAMETER_KIND_VARIABLE_ARGS:
{
...
int mcs_summarize_field_declarator_list(mc_syntax_node_list *syntax_declarators, field_declarator_info_list **field_declarators_list) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("mcs_summarize_field_declarator_list", __FILE__, __LINE__, &midge_error_stack_index);
  ...
  *field_declarators_list = (field_declarator_info_list *)malloc(sizeof(field_declarator_info_list));
  ...
  for (int  d = 0  ; d<syntax_declarators->count  ; ++d  ) {
    ...
    if (declarator_syntax->field_declarator.function_pointer) {
      mcs_copy_syntax_node_to_text(declarator_syntax->field_declarator.function_pointer->fptr_declarator.name, &declarator->function_pointer.identifier);
      if (declarator_syntax->field_declarator.function_pointer->fptr_declarator.fp_dereference) {
        declarator->function_pointer.fp_deref_count = declarator_syntax->field_declarator.function_pointer->fptr_declarator.fp_dereference->dereference_sequence.count;
      }
      else {
    ...
    append_to_collection((void ***)&(*field_declarators_list)->items, &(*field_declarators_list)->alloc, &(*field_declarators_list)->count, declarator);
  }
...
int mcs_summarize_type_field_list(mc_syntax_node_list *field_syntax_list, field_info_list **field_list) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("mcs_summarize_type_field_list", __FILE__, __LINE__, &midge_error_stack_index);
          ...
          break;          default:
{
      ...
      break;      case MC_SYNTAX_NESTED_TYPE_DECLARATION:
{
...
int register_sub_type_syntax_to_field_info(mc_syntax_node *subtype_syntax, field_info *field) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("register_sub_type_syntax_to_field_info", __FILE__, __LINE__, &midge_error_stack_index);
...
int mcs_construct_dependency(function_info *dependent_function, const char *identity) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("mcs_construct_dependency", __FILE__, __LINE__, &midge_error_stack_index);
...
int mcs_determine_code_dependencies(function_info *dependent_function, mc_syntax_node *code_block) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("mcs_determine_code_dependencies", __FILE__, __LINE__, &midge_error_stack_index);
...
int mcs_register_function_declaration(mc_syntax_node *function_ast, function_info **p_func_info) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("mcs_register_function_declaration", __FILE__, __LINE__, &midge_error_stack_index);
  ...
  if (function_ast->function.code_block) {
    ...
    for (p = 0    ; p<function_ast->function.parameters->count    ; ++p    ) {
      ...
      if (param->parameter.type_identifier&&param->parameter.type_identifier->type_identifier.identifier->type==MC_TOKEN_IDENTIFIER) {
...
int mcs_register_struct_declaration(mc_syntax_node *struct_ast, struct_info **p_struct_info) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("mcs_register_struct_declaration", __FILE__, __LINE__, &midge_error_stack_index);
...
int mcs_process_ast_root_children(mc_source_file_info *source_file, mc_syntax_node_list *children) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("mcs_process_ast_root_children", __FILE__, __LINE__, &midge_error_stack_index);
      ...
      break;      case MC_SYNTAX_FUNCTION:
{
      ...
      break;      case MC_SYNTAX_TYPE_ALIAS:
{
        char buf[1024];
          ...
          case MC_SYNTAX_STRUCT_DECL:
{
          ...
          break;          case MC_SYNTAX_ENUM_DECL:
{

          }
          break;          default:
          print_syntax_node(child->type_alias.type_descriptor, 0);
          MCerror(1185, "Unhandled type_alias-descriptor-syntax-type:%s", get_mc_syntax_token_type_name(child->type_alias.type_descriptor->type));
          break;        }

      }
      break;      case MC_SYNTAX_UNION_DECL:
      case MC_SYNTAX_STRUCT_DECL:
{
        struct_info *info;
      ...
      break;      case MC_SYNTAX_ENUM_DECL:
{
      ...
      break;      case MC_SYNTAX_PP_DIRECTIVE_DEFINE:
{

      }
      break;      case MC_SYNTAX_PP_DIRECTIVE_IFDEF:
{
        mcs_process_ast_root_children(source_file, child->preprocess_ifdef.groupopt);
      }
      break;      case MC_SYNTAX_PP_DIRECTIVE_IFNDEF:
{
        mcs_process_ast_root_children(source_file, child->preprocess_ifndef.groupopt);
      }
      break;      case MC_SYNTAX_PP_DIRECTIVE_INCLUDE:
      break;      default:
{
        switch ((mc_token_type)child->type) {
          ...
          MCerror(5313, "TODO");          case MC_TOKEN_SPACE_SEQUENCE:
...
int mcs_interpret_file(TCCInterpState *tis, const char *filepath) {
  int midge_error_stack_index;
  register_midge_stack_function_entry("mcs_interpret_file", __FILE__, __LINE__, &midge_error_stack_index);
  ...
  // Generate code (from the AST) with midge insertions integrated (stack call / error tracking)
{
  ...
  if (!strcmp("src/core/mc_source.c", filepath))   printf("\ngen-code:\n%s||\n", code);
  // MCerror(7704, "TODO");
